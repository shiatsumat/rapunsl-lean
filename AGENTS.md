...
- **`Fin` induction for `∏ᶠⁱ`/`∑ᶠⁱ` facts without a ready-made lemma.** First check the lemmas in `Algebra/Finiprod.lean` (`finiprod_rel`/`finisum_rel` etc.); when none of them fits (e.g. the fact moves a binder like `∃` through `∑ᶠⁱ`), `suffices` the statement over `Fin (n + 1)`, transfer the original goal to it with `finiprod_fin`/`finisum_fin`, and induct on `n` with `finiprod_fin_one` and `finiprod_fin_succ` (include `Nat.reduceAdd` in the base case's `simp only` so `Fin (0 + 1)` becomes `Fin 1`). If a dependent family `α : ι → Sort*` rides along, transport `∀ i, α i` across `Finitype.equivFin` with `Equiv.piCongrLeft'` and its simp lemma `Equiv.piCongrLeft'_symm_apply_apply`; build `Fin`-indexed witnesses with `Fin.cases` (base) and `Fin.lastCases` (step), reduced by `Fin.cases_zero`/`Fin.lastCases_last`/`Fin.lastCases_castSucc`. See `finisum_exists` in `RBI/Add.lean`.
- **`∃` witnesses in `RProp` entailment goals.** Consume an `∃` on the left with `apply exists_elim; intro x`; supply one on the right with `grw [←exists_intro w]`, which replaces the `∃` side with the instantiated body and closes the goal outright when both sides then coincide (after a `simp only`, finish with an explicit `rfl` — `simp` never closes an entailment). If elaborating `w` needs the expected type — an eliminator like `Fin.cases` fails with "failed to elaborate eliminator", or a bare lambda leaves its binder type unknown — pass `exists_intro`'s `Ψ` explicitly or annotate the lambda's binder type. Prefer `grw` over `entails_trans` for chaining entailments; the bare name doesn't resolve to the implicit-argument lemma you expect.
