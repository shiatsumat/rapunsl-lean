...
lemma Premultiset.prod.right_id (A : Premultiset α) (b : β) :
  A * Premultiset.singl b ≈ Premultiset.map (fun a => (a, b)) A := by
  exists fun (i, _) => i, fun i => (i, ());
  and_intros; { intros; trivial }; all_goals
    rintro ⟨_, _⟩; rfl

lemma Multiset.prod.right_id (A : Multiset α) (b : β) :
  A * Multiset.singl b = Multiset.map (fun a => (a, b)) A := by
  rw [←Quotient.out_eq A]; apply Quotient.sound;
  apply Premultiset.prod.right_id

lemma Multiset.prod.left_identity (a : α) (B : Multiset β) :
  Multiset.singl a * B = Multiset.map (fun b => (a, b)) B := by
  rw [Multiset.prod.comm, Multiset.prod.right_id, Multiset.map_map]; rfl
